...
image::{uml_diagrams_uri}/GCM-data_types.text.svg[id=rm_data_types_text, align="center"]
...
In a concrete instance of `DV_CODED_TEXT`, the following are the likely usages of the `_value_` and `_formatting_` attributes.
...
include::{uml_export_dir}/classes/{pkg}dvtext.adoc[]

include::{uml_export_dir}/classes/{pkg}termmapping.adoc[]

include::{uml_export_dir}/classes/{pkg}dvplaintext.adoc[]

include::{uml_export_dir}/classes/{pkg}dvcodedtext.adoc[]
