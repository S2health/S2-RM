...
[.text-center]
.gcm.data_types.ordered package - quantities
image::{uml_diagrams_uri}/GCM-data_types.ordered-quantity.svg[id=rm_data_types_quantity, align="center"]
...
In contrast to `Dv_ordinal`, instances of `Dv_quantified` and all its subtypes have precise numeric magnitudes. Accordingly, `Dv_quantified` introduces the properties `_magnitude_` and `_magnitude_status_`. The `_magnitude_` attribute is guaranteed to be available on any `Dv_quantified`, carrying the effective value, regardless of the particular subtype. The optional `_magnitude_status_` attribute can be used to provide a non-quantified indication of accuracy, and takes the following values:
...
The `Dv_measured` class provides a place to record these extra data items, and may be instantiated in any instance of a descendant `Dv_quantified` that represents a measurement.
