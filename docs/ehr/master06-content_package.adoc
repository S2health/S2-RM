...
The `content` package contains the `ContentItem` class, ancestor class of all content types, including `Section`, `Entry` and related types.

== Sections

The `Section` class enables a hierarchical heading structure, in which all individual headings are considered to belong to a "tree of headings". Each heading is an instance of the class `Section`, visible in the lower left side of <<rm_composition>>.
...
include::{uml_export_dir}/classes/{pkg}contentitem.adoc[]
