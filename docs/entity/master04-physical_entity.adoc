...
Following BFO, openEHR Physical entities are understood as entities existing within 3-dimensional space. They include material objects of any kind (Material_entity); spaces and spatial extents, and processes, including events etc.

[.text-center]
.rm.entity.physical_entity package
image::{uml_diagrams_uri}/GRM-entity.physical_entity.svg[id=rm_physical_entity, align="center"]
...
include::{uml_export_dir}/classes/{pkg}material_point.adoc[]

include::{uml_export_dir}/classes/{pkg}material_line.adoc[]

include::{uml_export_dir}/classes/{pkg}material_surface.adoc[]
