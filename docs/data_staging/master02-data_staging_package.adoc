...
The figure below illustrates the `rm.intg.data_staging` package, which contains a single class
...
[.text-center]
.rm.intg.data_staging Package
image::{uml_diagrams_uri}/RM-data_staging.svg[id=data_staging_package, align="center"]
...
In other words, a repository of `Generic_entries` (within appropriate `Composition` structures)
