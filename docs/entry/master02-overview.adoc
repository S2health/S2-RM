...
The `_authorization_actions_` attribute is used to record any authorization action, generally signing or counter-signing. Each such action is assumed to advance the Entry through a lifecycle, recorded in the Entry attriubte `_lifecycle_state_`, and defined by the Entry State Machine, which is illustrated below.

[.text-center]
.Entry state machine
image::{uml_diagrams_uri}/GCM-Entry_state_machine.svg[id=Entry_state_machine, align="center"]
...
The `Care_entry` class is an abstract precursor of classes that express information of any clinical activity in the patient care process. It includes two attributes particular to all clinical entries, namely `_protocol_` and `_guideline_id_`. These attributes enable the 'how' and 'why' aspects of any clinical recording to be expressed. Protocol is often recorded for Observations, e.g. staining method in microscopy, and Instructions, e.g. Bruce protocol for ECG test.

TODO: review guideline_id
