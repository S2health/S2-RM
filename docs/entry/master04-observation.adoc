...
Lab results are represented using `Observation`, but with the `_result_time_` property set. A lab observation is fundamentally different from an instantaneous observation, since it is made on a _specimen_ that acts as a proxy for the subject.
