...
The openEHR `Entry` classes are shown below. There are five concrete subtypes: `Admin_entry`, `Observation`, `Assertion`, `Order` and `ACTION`, of which the latter four are kinds of `Care_entry`.
...
The openEHR approach to these challenges is to use the Entry type `Order`, its subpart
