...
example, within a `Composition`, `Entry` instances (i.e. `Observations`, `Assertions` etc.) are
...
`Cluster` instances) might also be archetype root points, depending on how archetypes
