...
== Ontological Information Architecture

S2 adopts an ontological approach information architecture, which results in two categories of information entity in its model universe:

* 'digital twins' of ontic (real world) entities;
* epistemic information objects resulting from observation, inference, issuing or orders and other information-generating interactions with the real world.

One consequence of this approach is that elements in S2 domain models are all marked with a code that indicates what the element is about.
